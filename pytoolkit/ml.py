...
def print_classification_metrics(y_true, proba_pred, average='macro', print_fn=None):
    ...
    try:
        ...
        if true_type == 'binary':  # binary
            ...
            acc = sklearn.metrics.accuracy_score(y_true, y_pred)
            prec, rec, f1, _ = sklearn.metrics.precision_recall_fscore_support(y_true, y_pred, labels=labels, average=average)
            auc = sklearn.metrics.roc_auc_score(ohe_true, proba_pred, average=average)
            ap = sklearn.metrics.average_precision_score(ohe_true, proba_pred, average=average)
