...
def test_bigbang(tmpdir):
    ...
    # 学習
    train_data, train_labels = pytoolkit.lgb.load_labeled_data(
    ...
    # 検証
    test_data, test_labels = pytoolkit.lgb.load_labeled_data(test_data_path, "variety")
    model = pytoolkit.lgb.load(model_dir)
    score = model.evaluate(test_data, test_labels)
    assert 0.99 <= score["auc"] <= 1.0

    # 推論
    input_data = pytoolkit.lgb.load_unlabeled_data(test_data_path)
