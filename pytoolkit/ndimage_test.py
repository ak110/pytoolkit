...
def test_filters(data_dir, check_dir):
    ...
    filters = [
        ...
        (1, "contrast_l", lambda rgb: tk.ndimage.contrast(rgb, 0.5)),
        (1, "contrast_h", lambda rgb: tk.ndimage.contrast(rgb, 1.5)),
        (
            1,
            "hue_lite_b",
            lambda rgb: tk.ndimage.hue_lite(
                rgb, np.array([0.95, 0.95, 1.05]), np.array([-8, -8, +8])
            ),
        ),
        (
            1,
            "hue_lite_g",
            lambda rgb: tk.ndimage.hue_lite(
                rgb, np.array([0.95, 1.05, 0.95]), np.array([-8, +8, -8])
            ),
        ),
        (
            1,
            "hue_lite_r",
            lambda rgb: tk.ndimage.hue_lite(
                rgb, np.array([1.05, 0.95, 0.95]), np.array([+8, -8, -8])
            ),
        ),
        ...
        (
            0,
            "random_alpha",
            lambda rgb: tk.ndimage.erase_random(rgb, random, alpha=0.125),
        ),
        (
            0,
            "random_erasing",
            lambda rgb: tk.ndimage.erase_random(
                rgb, random, bboxes=np.array([[64, 64, 128, 128]])
            ),
        ),
        (
            0,
            "random_alpha",
            lambda rgb: tk.ndimage.erase_random(
                rgb, random, bboxes=np.array([[64, 64, 128, 128]]), alpha=0.125
            ),
        ),
        ...
        (
            0,
            "gt_flip_h",
            lambda rgb: tk.ndimage.geometric_transform(rgb, 256, 256, flip_h=True),
        ),
        (
            0,
            "gt_flip_v",
            lambda rgb: tk.ndimage.geometric_transform(rgb, 256, 256, flip_v=True),
        ),
        (
            0,
            "gt_zoomin_h",
            lambda rgb: tk.ndimage.geometric_transform(rgb, 256, 256, scale_h=1.5),
        ),
        (
            0,
            "gt_zoomout_h",
            lambda rgb: tk.ndimage.geometric_transform(rgb, 256, 256, scale_h=0.75),
        ),
        (
            0,
            "gt_zoomin_v",
            lambda rgb: tk.ndimage.geometric_transform(rgb, 256, 256, scale_v=1.5),
        ),
        (
            0,
            "gt_zoomout_v",
            lambda rgb: tk.ndimage.geometric_transform(rgb, 256, 256, scale_v=0.75),
        ),
        (
            0,
            "gt_rotate_1",
            lambda rgb: tk.ndimage.geometric_transform(rgb, 256, 256, degrees=+15),
        ),
        (
            0,
            "gt_rotate_2",
        ...
        (
            0,
            "gt_pos_s150_10",
            lambda rgb: tk.ndimage.geometric_transform(
                rgb, 256, 256, scale_h=1.5, scale_v=1.5, pos_h=1, pos_v=0
            ),
        ),
        (
            0,
            "gt_pos_s150_01",
            lambda rgb: tk.ndimage.geometric_transform(
                rgb, 256, 256, scale_h=1.5, scale_v=1.5, pos_h=0, pos_v=1
            ),
        ),
        (
            0,
            "gt_pos_s075_10",
            lambda rgb: tk.ndimage.geometric_transform(
                rgb, 256, 256, scale_h=0.75, scale_v=0.75, pos_h=1, pos_v=0
            ),
        ),
        (
            0,
            "gt_pos_s075_01",
            lambda rgb: tk.ndimage.geometric_transform(
                rgb, 256, 256, scale_h=0.75, scale_v=0.75, pos_h=0, pos_v=1
            ),
        ),
        (
            0,
            "gt_pos_s100_10",
            lambda rgb: tk.ndimage.geometric_transform(
                rgb, 256, 256, scale_h=1.0, scale_v=1.0, pos_h=1, pos_v=0
            ),
        ),
        (
            0,
            "gt_pos_s100_01",
            lambda rgb: tk.ndimage.geometric_transform(
                rgb, 256, 256, scale_h=1.0, scale_v=1.0, pos_h=0, pos_v=1
            ),
        ),
        (
            0,
...
def test_cut_mix(data_dir, check_dir):
    ...
    image, label = tk.ndimage.cut_mix((rgb1, np.zeros(())), (rgb2, np.ones(())))
